...
class Supervisor extends BaseApprover {
  ...
  override approveRequest(amount: number): void {
    if (amount <= 1000) {
      console.log('Estado solicitud: %cAprobada por supervisor', COLORS.green);
      return;
    }

    console.log('Estado solicitud: %cMoviendo solicitud a siguiente nivel de aprobacion', COLORS.yellow);
    this.next(amount);
  }
}
...
class Manager extends BaseApprover {
  ...
  override approveRequest(amount: number): void {
    if (amount <= 5000) {
      console.log('Estado solicitud: %cAprobada por manager', COLORS.orange);
      return;
    }
...
class Director extends BaseApprover {
  ...
  override approveRequest(amount: number): void {
    console.log(`Estado solicitud: %cAprobada por director, monto $${amount}`, COLORS.red);
    return;
  }
}
